...
# Fungsi untuk melakukan masking pada gambar terenkripsi
def mask_image(encrypted_image):
    # Konversi data gambar menjadi array numpy
    image_array = np.frombuffer(encrypted_image, dtype=np.uint8)
    
    # Melakukan masking pada setiap byte gambar
    masked_image_array = np.bitwise_xor(image_array, np.random.randint(0, 256, size=len(image_array), dtype=np.uint8))
    
    # Mengonversi array kembali menjadi data bytes
    masked_image = bytes(masked_image_array)
    
    return masked_image

# Fungsi untuk mengenkripsi gambar dan melakukan masking
def encrypt_image(image_data, key):
    ...
    # Menggabungkan IV dengan ciphertext
    encrypted_image = iv + ciphertext
    
    # Melakukan masking pada gambar terenkripsi
    masked_encrypted_image = mask_image(encrypted_image)
    
    return masked_encrypted_image

# Menulis gambar terenkripsi yang telah dimasked ke dalam file PNG
def write_masked_image(filename, data, width, height):
...
def decrypt_masked_image(masked_image, key):
    ...
    # Melakukan dekripsi pada gambar yang telah dimasked
    decrypted_image = decrypt_image(unmasked_image, key)
    
    return decrypted_image

# Fungsi untuk melakukan dekripsi pada gambar
def decrypt_image(encrypted_image, key):
    # Mengambil IV dari gambar terenkripsi
    iv = encrypted_image[:16]
    # Mengambil ciphertext dari gambar terenkripsi
    ...
    # Menghapus padding dari data dekripsi
    decrypted_data = unpad(decrypted_data, AES.block_size)
    
    return decrypted_data

# Fungsi untuk menulis gambar yang telah didekripsi ke dalam file
def write_decrypted_image(filename, data):
    with open(filename, 'wb') as f:
        f.write(data)

# Fungsi untuk melakukan unmasking pada gambar yang telah dimasked
def unmask_image(masked_image):
    # Karena masking dilakukan dengan operasi XOR, unmasking dapat dilakukan dengan operasi XOR kembali
    unmasked_image_array = np.bitwise_xor(np.frombuffer(masked_image, dtype=np.uint8), np.random.randint(0, 256, size=len(masked_image), dtype=np.uint8))
    # Mengonversi array kembali menjadi data bytes
    unmasked_image = bytes(unmasked_image_array)
...
def encrypt():
    try:
        ...
        # Melakukan enkripsi pada gambar dan masking
        masked_encrypted_image = encrypt_image(image_data, key.encode())

        # Dapatkan timestamp saat ini sebagai bagian dari nama file
        timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
        output_filename = f"static/encrypted_{timestamp}.png"

        # Menyimpan gambar terenkripsi yang telah dimasked
        write_masked_image(output_filename, masked_encrypted_image, width, height)
...
# Route untuk melakukan dekripsi
...
def decrypt():
    try:
        ...
        # Membaca data gambar terenkripsi yang telah dimasked
        encrypted_image_data = encrypted_image_file.read()

        # Melakukan dekripsi pada gambar terenkripsi yang telah dimasked
        decrypted_image = decrypt_masked_image(encrypted_image_data, key.encode())

        # Dapatkan timestamp saat ini sebagai bagian dari nama file
        timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
        output_filename = f"static/decrypted_{timestamp}.png"

        # Menyimpan gambar yang telah didekripsi
        write_decrypted_image(output_filename, decrypted_image)
